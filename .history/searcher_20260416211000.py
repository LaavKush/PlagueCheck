...
def extract_key_phrases(text: str, num_phrases: int = 3) -> list[str]:
    ...
    """
    sentences = re.split(r'(?<=[.!?])\s+', text.strip())
    # Adjusted filter to allow slightly longer technical phrases
    good = [
        s.strip() for s in sentences
        if 6 <= len(s.split()) <= 25 and re.search(r'[a-zA-Z]', s)
    ]
    # Pick spread-out sentences
    if not good:
        words = text.split()
...
def fetch_page_text(url: str, max_chars: int = 8000) -> str:
    """
    Fetch a webpage and extract clean body text with better anti-blocking.
    """
    # Expanded headers to mimic a real browser more closely
    browser_headers = {
    ...
    try:
        # Using a session can sometimes help with 403s
        session = requests.Session()
        resp = session.get(url, headers=browser_headers, timeout=10)
        
        # If we still get a 403, it's a hard block, but this helps with GFG and others
        resp.raise_for_status()
        ...
        # Target GeeksforGeeks and general article structures specifically
        main = (
