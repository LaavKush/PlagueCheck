...
def fetch_page_text(url: str, max_chars: int = 8000) -> str:
    ...
    try:
        ...
        # Removed 'aside' from decomposition as GFG uses it for some content
        for tag in soup(["script", "style", "nav", "footer", "header", "form"]):
            tag.decompose()

        # Try to find common content containers first
        main = soup.find("article") or soup.find("div", class_="entry-content") or soup.find("main") or soup.find("body")
        if not main:
            return ""
